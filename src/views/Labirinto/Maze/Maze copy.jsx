...
function Maze(props) {
  ...
  const blocksLogicLevel5 = () => {
    ...
    let xWeAreRepeating = [];
    xBlocks.map((pCommand, pIndex) => {
      if (pCommand === "repeat" || xWeAreRepeating.length === 1) {
        console.log("primeiro if");
        xWeAreRepeating.push(pCommand);
      } else if (xWeAreRepeating.length === 2) {
        console.log("segundo if");
        let xMove = "";
        let xRepetitions = 0;
        if (typeof pCommand === "number") {
          xMove = xWeAreRepeating[1];
          xRepetitions = pCommand;
        } else {
          xMove = pCommand;
          xRepetitions = xWeAreRepeating[1];
        }
        const xRepeatArray = Array(xRepetitions).fill().map(Math.random);
        xRepeatArray.map((random, index) => {
